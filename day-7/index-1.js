...
const canSolve = (target, numbers, ops = []) => {
  console.log(`target=${target} numbers=${numbers}`);
  ...
  for (const operand of operands) {
    ...
    if (newTotal === target && numbers.length === 2) {
      console.log(`SOLVED with ops=${ops.map((op) => op.name)}`);
      return true;
    }
    ...
    if (canSolve(target, [newTotal, ...numbers.slice(2)], [...ops, operand])) {
      return true;
    }
  }
...
console.log(equations.filter((equation) => equation.canSolve === true).reduce((acc, equation) => acc + equation.target, 0));
console.dir(equations.filter((equation) => equation.canSolve === true && equation.numbers.length < 5), {depth: null});
